...
from utils.helpers import format_currency, get_days_remaining, can_delete
...
_PAGE_META = {
    "vs_actual": (
        "Budget vs Actual",
        "Compare spending against your category limits for each pay cycle",
    ),
    "sinking": (
        "Sinking Funds",
        "Save gradually for large planned expenses",
    ),
    "forecast": (
        "Cashflow Forecast",
        "Project income, bills, and surplus across months or pay cycles",
    ),
}


def show_budgeting(household_id: int, page: str | None = None):
    """
    Renders budget management views.
    page: None = all tabs (legacy); or 'vs_actual', 'sinking', 'forecast'.
    """
    if page and page in _PAGE_META:
        title, subtitle = _PAGE_META[page]
        ...
        st.markdown("<h1 class='app-title'>This Pay Cycle</h1>", unsafe_allow_html=True)
        st.markdown(
            "<p class='app-subtitle'>See what's left until payday, set limits, and plan sinking funds</p>",
            unsafe_allow_html=True,
        )

    def _active(section: str) -> bool:
        return page is None or page == section

    currency = st.session_state.get("household_currency", "FJD")

    if page is None:
        tab_bud, tab_sink, tab_forecast, tab_prefs = st.tabs([
            "📊 Cycle Budgets", "🏺 Sinking Funds", "🔮 Interactive Forecaster", "⚙️ Preferences",
        ])
    else:
        tab_bud = tab_sink = tab_forecast = tab_prefs = None
    ...
    with get_db() as db:
        ...
        # CYCLE BUDGETS TAB
        # ----------------------------------------------------
        if _active("vs_actual"):
            _bud_ctx = tab_bud if page is None else st.container()
            with _bud_ctx:
                if not all_periods:
                    ...
                    if not budget:
                        st.info("No budget configured for this period yet.")
                        if role == "viewer":
                            st.caption("Ask a household owner or partner to set up budget limits.")
                        elif st.button("Create Budget for This Period", type="primary", key=f"create_bud_{selected_period.id}"):
                            budget = Budget(
                            ...
                            db.commit()
                            db.refresh(budget)
                            st.success("Budget created! Configure your category limits below.")
                            st.rerun()

                    # Configure category budget limits
                    if budget and role == "viewer":
                    ...
                    st.write("")
                    if not budget:
                        st.info("Configure a budget for this period to see variance analysis.")
                    else:
        ...
        # ----------------------------------------------------
        if _active("sinking"):
            _sink_ctx = tab_sink if page is None else st.container()
            with _sink_ctx:
        ...
        # ══════════════════════════════════════════════════════
        if _active("forecast"):
            _fore_ctx = tab_forecast if page is None else st.container()
            with _fore_ctx:
                ...
                if not income_items and not expense_items:
                    ...
                    with col_l:
                        ...
                        if not income_items:
                            ...
                            st.dataframe(pd.DataFrame(inc_table), hide_index=True, width="stretch")
        ...
        if page is None:
            with tab_prefs:
                ...
                show_pay_settings(household_id, embedded=True)
